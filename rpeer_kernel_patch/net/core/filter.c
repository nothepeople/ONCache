...
int skb_do_redirect(struct sk_buff *skb)
{
	...
	struct net_device *dev;
	u32 flags = ri->flags;
	u32 tgt_index = ri->tgt_index;
	ri->tgt_index = 0;
	ri->flags = 0;
	// printk(KERN_INFO "Start skb_do_redirect");
	if (flags & BPF_F_RPEER) {
		// printk(KERN_INFO "Start BPF_F_RPEER");
		const struct net_device_ops *ops = skb->dev->netdev_ops;
		if (unlikely(!ops->ndo_get_peer_dev ||
			     skb_at_tc_ingress(skb)))
			goto out_drop;
		// printk(KERN_INFO "dev = ops->ndo_get_peer_dev(skb->dev);");
        dev = ops->ndo_get_peer_dev(skb->dev);
        struct net *net_main = dev_net(dev);
    	dev = dev_get_by_index_rcu(net_main, tgt_index);
		if (unlikely(!dev ||
				!(dev->flags & IFF_UP) ||
				net_eq(net, dev_net(dev))))
			goto out_drop;
		// printk(KERN_INFO "dev get finished");
	} else {
		dev = dev_get_by_index_rcu(net, tgt_index);
	}
...
BPF_CALL_2(bpf_redirect_rpeer, u32, ifindex, u64, flags)
{
	// printk(KERN_INFO "Start bpf_redirect_rpeer");
	struct bpf_redirect_info *ri = this_cpu_ptr(&bpf_redirect_info);

	if (unlikely(flags))
		return TC_ACT_SHOT;

	ri->flags = BPF_F_RPEER;
	ri->tgt_index = ifindex;
	// printk(KERN_INFO "Finished bpf_redirect_rpeer");
	return TC_ACT_REDIRECT;
}

static const struct bpf_func_proto bpf_redirect_rpeer_proto = {
	.func           = bpf_redirect_rpeer,
	.gpl_only       = false,
	.ret_type       = RET_INTEGER,
	.arg1_type      = ARG_ANYTHING,
	.arg2_type      = ARG_ANYTHING,
